...
@section('content')
   <div class="max-w-3xl mx-auto">
      <h2 class="text-xl font-semibold mb-4">{{ __('app.edit_warehouse') }}</h2>
      ...
      <form method="POST" action="{{ route('warehouses.update', $warehouse) }}" class="bg-white p-4 rounded shadow">
         ...
         <div class="grid grid-cols-3 gap-4 mb-3">
            <div>
               ...
               <input name="city" value="{{ old('city', $warehouse->city) }}"
            ...
            <div>
               <label class="inline-flex items-center">
                  <input type="checkbox" name="is_active" class="mr-2"
                     ...
                     @if ($warehouse->is_default) onclick="return confirm('{{ __('app.default_warehouse_cannot_deactivate') }}');" @endif />
                  {{ __('app.active') }}
               </label>
            ...
            <div>
               ...
               @if ($warehouse->is_default)
                  <p class="text-xs text-blue-600 ml-6 mt-1">
                     <strong>{{ __('app.info') }}:</strong> {{ __('app.warehouse_currently_default') }}
                  </p>
               @endif
            </div>
         </div>
         ...
         <div class="flex gap-2">
            <button type="submit" class="px-3 py-2 bg-primary text-white rounded">{{ __('app.update') }}</button>
