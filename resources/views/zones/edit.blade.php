...
@section('title', __('app.edit_zone'))

@section('content')
   <div class="max-w-3xl mx-auto">
      <div class="flex items-center justify-between mb-4">
         <h2 class="text-xl font-semibold">{{ __('app.edit_zone') }}: {{ $zone->name }}</h2>
         ...
         <form action="{{ route('zones.update', $zone) }}" method="POST">
            ...
            <div class="mb-4">
               ...
               <select name="warehouse_id" id="warehouse_id" required
                  ...
                  @foreach ($warehouses as $warehouse)
                     <option value="{{ $warehouse->id }}" {{ old('warehouse_id', $zone->warehouse_id) == $warehouse->id ? 'selected' : '' }}>
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
               <div>
                  ...
                  <input type="text" name="code" id="code" value="{{ old('code', $zone->code) }}" required maxlength="20"
               ...
               <div>
                  ...
                  <input type="text" name="name" id="name" value="{{ old('name', $zone->name) }}" required maxlength="100"
            ...
            <div class="mb-4">
               ...
               <select name="type" id="type" required
                  ...
                  @foreach ($types as $type)
                     <option value="{{ $type }}" {{ old('type', $zone->type) == $type ? 'selected' : '' }}>
                        {{ ucfirst($type) }}
                     </option>
                  @endforeach
               </select>
            ...
            <div class="mb-4">
               ...
               <textarea name="description" id="description" rows="3" maxlength="500"
                  class="w-full border rounded px-3 py-2 @error('description') border-red-500 @enderror">{{ old('description', $zone->description) }}</textarea>
               @error('description')
                  <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
               @enderror
            </div>
